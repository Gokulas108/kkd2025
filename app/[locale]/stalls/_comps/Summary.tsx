	...
	const StepProcess = () => {
		return (
			<ol className="flex justify-center w-full text-xs text-gray-900 font-medium sm:text-base">
				...
				<li className="flex w-full relative text-slate-500 justify-center  after:content-['']  after:w-full after:h-0.5  after:bg-slate-800 after:inline-block after:absolute lg:after:top-5 after:top-3 after:left-1/2">
					<div className="block whitespace-nowrap text-wrap text-center z-10">
						<span className="w-6 h-6 bg-slate-800 border-2 border-slate-800 rounded-full flex justify-center items-center mx-auto mb-3 text-sm text-white lg:w-10 lg:h-10">
							2
						</span>{" "}
						{t("personal_details")}
					</div>
				...
				<li className="flex w-full relative text-gray-800 justify-center">
					<div className="block whitespace-nowrap text-wrap text-center z-10">
						<span className="w-6 h-6 bg-white border-2 border-slate-800 rounded-full flex justify-center items-center mx-auto mb-3 text-sm  lg:w-10 lg:h-10">
							3
						</span>{" "}
						{t("confirmation")}
					</div>
				...
				<div className="w-full mx-4 rounded bg-gray-50 px-6 pt-8 shadow-lg">
						...
						<table className="w-full text-left">
							<thead>
								<tr className="flex">
									...
									<th className="min-w-[44px] py-2">{t("age")}</th>
								</tr>
							</thead>
							<tbody>
								{summary.members.map((mem: any, index: number) => (
									<tr className="flex py-1" key={index}>
										...
										<td className="min-w-[44px]">{mem.age}</td>
									</tr>
								))}
							</tbody>
						</table>
					...
					<div className="flex flex-col gap-3 pb-6 pt-2 text-xs">
						<table className="w-full text-left">
							<thead>
								<tr className="flex">
									...
									<th className="min-w-[70px] py-2 text-center">{t("qty")}</th>
						...
						<div className="py-4 justify-center items-center flex flex-col gap-2">
							<button
								className="flex w-full py-2 gap-2 bg-slate-800 text-slate-100 px-4 rounded justify-center items-center"
								type="submit"
								onClick={onSubmit}
							>
								{loading ? (
